...
[![Version](https://img.shields.io/badge/version-1.2.1-blue)](https://github.com/ry-ops/eagle-scout/releases/tag/v1.2.1)
...
```bash
docker pull ryops/eagle-scout:1.2.1
```

Also available on GitHub Container Registry:

```bash
docker pull ghcr.io/ry-ops/eagle-scout:1.2.1
```
...
Download from [Releases](https://github.com/ry-ops/eagle-scout/releases/tag/v1.2.1) — available for Linux, macOS, and Windows (amd64/arm64).
...
```json
{
  "mcpServers": {
    "eagle-scout": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-v", "/var/run/docker.sock:/var/run/docker.sock", "ryops/eagle-scout:1.2.1"]
    }
  }
}
```
